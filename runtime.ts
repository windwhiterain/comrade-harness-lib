...
import type { Flow, HarnessConfig, MemoryStore, SessionStore, StreamEvent } from "./types";
...
export function createHarness(cfg: HarnessConfig) {
  ...
  // 无记录会话的模型锚点：core 启动时的初始选择（会话化之后"全局当前"会被切换污染，不能当默认）
  const initialModel = selector ? selector.state() : null;
  ...
  Bun.serve({
    ...
    async fetch(req) {
      ...
      if (url.pathname === "/api/models" && selector) {
        if (req.method === "GET") {
          // session 参数：该会话记过的模型优先；无记录 = core 初始模型（不是被其他会话切换污染的"当前"）
          const sid = url.searchParams.get("session");
          let current = initialModel ?? selector.state();
      ...
      if (url.pathname === "/api/messages" && req.method === "POST") {
        const { text, session, at, regen } = (await req.json()) as
          { text?: string; session?: unknown; at?: unknown; regen?: unknown };
        const isRegen = regen === true;
        if (!isRegen && !text?.trim()) return json({ error: "空消息" }, 400);
        const sid = resolveSession(session);
        if (busySessions.has(sid)) return json({ error: `${CORE_ID} 的会话 ${sid} 正忙，请等上一条消息结束` }, 409);
        const missing = llmMissing();
      ...
      if (url.pathname === "/api/chat" && req.method === "POST") {
        ...
        const { message, session } = (await req.json()) as { message?: string; session?: unknown };
