...
export function createHarness(cfg: HarnessConfig) {
  ...
  Bun.serve({
    ...
    async fetch(req) {
      ...
      // 消息编辑 ops（2026-08-16 单一编辑面，取代 delete/truncate/update/clear 四个特例端点）：
      // 原子批量应用（任一 op 失败整体回滚）；op 词汇见 types.ts 的 MsgOp——路径寻址、tool 消息删除
      // 联动（连带移除 owner 的 tool_call）、tool_calls 删除连带删关联 tool 消息引用都在 store 内置。
      // 状态变更一律 POST（CSRF 防护，与 daemon 控制面同策略）。
      if (url.pathname === "/api/messages/ops" && req.method === "POST") {
        const { ops, session } = (await req.json()) as { ops?: unknown; session?: unknown };
        if (!Array.isArray(ops) || ops.length === 0) return json({ error: "需要 ops 数组" }, 400);
        const mem = memFor(resolveSession(session));
        if (!mem.applyOps) return json({ error: "当前存储不支持消息编辑 ops" }, 400);
        const res = mem.applyOps(ops as import("./types").MsgOp[]);
        if ("error" in res) return json({ error: res.error }, 400);
