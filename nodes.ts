...
/** 记忆节点：读历史进上下文——**严格重建**：存储就是 OpenAI 标准消息（一次 LLM 响应一条，
 *  content / reasoning_content / tool_calls 三字段一体），读出后原样回传，不拆不并不改写。
 *  消息的 meta（过程日志，2026-08-16 起内嵌）**剥掉**——只存不喂，绝不进模型上下文。
 *  日志行（role="step" 的 cut/llm，旧数据/无主日志，只给 UI 展示）与旧格式数据（拆行 step / 纯文本行）
 *  跳过，不进上下文。默认**全量**——不过滤、不截断；显式传 limit 才窗口化（最近 limit 条）。 */
export function loadHistory(memory: MemoryStore, limit?: number): ChatMessage[] {
  ...
  for (const m of slice) {
    if (m.role === "step") continue; // 日志行（cut/llm）：只给 UI 展示，不进 LLM 上下文
    const { id, ts, meta, ...msg } = m;
...
/** 记忆节点：把这一轮对话写入历史（user 先、assistant 后，标准消息）。 */
export function saveHistory(memory: MemoryStore, userText: string, reply: string): void {
  memory.insert({ role: "user", content: userText });
  memory.insert({ role: "assistant", content: reply });
}
