...
/** 从模型 content 里剥出开头的 `<think>…</think>` 块（deepseek-v4-pro 系会把思考直接写进 content）。
 *  返回 { think, text }：think = 块内文本（无标签），text = 剩余部分；没有块时 think = null。 */
export function splitThinkBlock(content: string): { think: string | null; text: string } {
  const m = content.match(/^\s*<think>([\s\S]*?)<\/think>\s*/);
  if (!m) return { think: null, text: content };
  return { think: m[1], text: content.slice(m[0].length) };
}

/** 记忆节点：读历史进上下文，**重建为标准 message 格式**（LLM 请求的标准形状，不是 JSON 文本）：
 *  - user → user；agent → assistant（文本）
 *  - step 行（过程痕迹，JSON 存库）：
 *    - think → assistant 文本消息（模型自己的思考，全量保留；`<think>` 标签包装剥掉）
...
export function loadHistory(memory: MemoryStore, limit?: number): ChatMessage[] {
  ...
  for (const m of slice) {
    if (m.role === "user") {
      ...
      if (s?.type === "think") {
        const { think, text } = splitThinkBlock(s.content);
        messages.push({ role: "assistant", content: [think, text.trim()].filter(Boolean).join("\n") });
      } else if (s?.type === "tool") {
        const id = `step-${m.id}`;
        messages.push({
          role: "assistant",
