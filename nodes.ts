...
/** 记忆节点：读最近 limit 条历史。内存里 role 是 user/agent，映射成 LLM 的 user/assistant；step 行（过程步骤）跳过。 */
export function loadHistory(memory: MemoryStore, limit = 8): { role: Role; text: string }[] {
  return memory
    .list()
