 ...
 *  所有 core 完全平等——standard/dsh-minimal 与用户 fork 的 core 一样可以修改自己、commit、reload；
 *  没有硬权限，agent 就是开发者，安全网是 git。 */
export function buildSystemPrompt(coreId: string): string {
  return `你运行在 ${coreId} core 里。core 是一个普通的 Bun/TS 服务，你是它的 agent：可以自由修改它——包括修改你自己：改完 tsc + reload 自己后，你会用新代码继续工作。
...
6. 你可以 read 任何 core 的文件作为参考。
7. git 快照与回滚由 daemon 管理，需要时调用 snapshot / rollback 工具。每次完成一轮有效改动后打一次快照即可，不要重复打。
8. 少而准：每一步只做一件必要的事；已经确认过的信息不要重复验证。
9. 完成任务后用 done(message) 收尾，message 用中文简要总结。`;
}

/** 记忆节点：读历史进上下文，**重建为标准 message 格式**（LLM 请求的标准形状，不是 JSON 文本）：
 *  - user → user；agent → assistant（文本）
 *  - step 行（过程痕迹，JSON 存库）：
 *    - think → assistant 文本消息（模型自己的思考，原样全量保留——思考只来自 reasoning_content，
 *      不做任何标签/格式处理；deepseek-v4-pro 系只出 reasoning_content，不往 content 里写思考）
 *    - tool → assistant.tool_calls（id = `step-<行id>`，arguments 原样 JSON 字符串）+ 紧随的 tool 角色消息
 *      （tool_call_id 对应，content = 工具结果）——工具痕迹以 API 标准工具消息进上下文，模型不会模仿 JSON。
 *  默认**全量**——不过滤、不截断；显式传 limit 才窗口化（最近 limit 条；0 = 不读历史，每轮独立）。
 *  窗口按行切，工具消息对由单条 step 行生成，不会被切散。 */
export function loadHistory(memory: MemoryStore, limit?: number): ChatMessage[] {
  ...
  for (const m of slice) {
    if (m.role === "user") {
      ...
      let s: HistoryStep | null = null;
      try {
        s = JSON.parse(m.text) as HistoryStep;
      } catch {
        s = null;
      }
      if (s?.type === "think") {
        ...
        // LLM 用量统计步骤：只给 UI 展示，不进入 LLM 上下文
      } else if (s?.type === "tool") {
        const id = `step-${m.id}`;
        messages.push({
          role: "assistant",
          content: "",
          tool_calls: [{ id, type: "function", function: { name: s.name, arguments: s.args } }],
        });
        messages.push({ role: "tool", tool_call_id: id, content: s.result ?? "（已停止）" });
      } else {
        messages.push({ role: "assistant", content: m.text }); // 解析失败/未知形状：原样兜底，不丢数据
      }
    } else {
      messages.push({ role: "assistant", content: m.text }); // 未知角色：原样兜底，不丢数据
    }
  }
  return messages;
}
 ...
 *  历史条目兼容两种形状：loadHistory 产出的标准 message（含 tool_calls/tool 角色）原样通过；
 *  旧形状 {role, text}（既有 fork 的调用）映射为 content。 */
export function composeMessages(system: string, history: HistoryEntry[], userText: string): ChatMessage[] {
  return [
    { role: "system", content: system },
    ...history.map((h) => ("text" in h ? { role: h.role, content: h.text } : h)),
