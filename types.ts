...
export interface ChatMessage {
  ...
  /** 过程日志（2026-08-16 起）：llm 统计 / 截断诊断归位进所属 assistant 消息（渲染随卡、删除卡片连带删除、
   *  孤儿不可能）。**只存不喂**——loadHistory 重建 LLM 输入时剥掉，绝不进模型上下文。 */
  meta?: { steps?: HistoryStep[] };
}
...
/** 消息编辑操作（2026-08-16 单一编辑面：取代 update/clear/delete/truncate 四个特例端点）。
 *  insert：插入消息（after 缺省 = 末尾；多消息依序连续插入，返回各新 id）
 *  update：按路径改字段；remove: true = 删除该路径（content 置空 = ""；reasoning_content = null；
 *    tool_calls 删除 = 连带删本会话中关联 tool 消息的引用）。路径段：content / reasoning_content /
 *    tool_calls / <tcId> / function / arguments（tool_calls 数组段按元素 id 匹配，不按下标）。
 *  remove：删消息（tool 消息 = 连带移除 owner 的 tool_call——数据模型不变量）。
 *  truncate：删 from 及其后。
 *  move：把消息移到 after 之后（after 缺省 = 末尾；已在目标位置 = 无操作）。 */
export type MsgOp =
  | { op: "insert"; after?: number; messages: StoredMessage[] }
  | { op: "update"; id: number; path: string; value?: unknown; remove?: boolean }
  | { op: "remove"; id: number }
  | { op: "truncate"; from: number }
  | { op: "move"; id: number; after?: number };
...
export interface MemoryStore {
   ...
   *  删 tool 消息时连带从 owner assistant 移除对应 tool_call（数据模型不变量：tool 消息与 tool_call
   *  必须成对；owner 不在本会话则只删引用）。 */
  ...
  /** 在 id 之后插入一条消息（中间插入，后续引用的 ord 自动取邻居间新键；返回新消息 id，便于连续插入）。可选。 */
  insertAfter?(id: number, msg: StoredMessage): number | null;
  /** 批量编辑操作（2026-08-16 起，取代 updateField/clearField 的封闭字段集合）：原子应用（任一失败
   *  整体回滚）。返回 { results } = 与 ops 一一对应的新消息 id（insert/update 产生新 id，其余 null），
   *  或 { error }。路径寻址见 MsgOp；**不校验改得合不合法**（用户自己决定改什么——超高自由度上下文
   *  编辑器）。可选：不支持时编辑请求被拒。 */
  applyOps?(ops: MsgOp[]): { results: (number | null)[] } | { error: string };
}
