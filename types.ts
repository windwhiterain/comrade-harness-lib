...
/**
 * 过程步骤：agent 在回答用户前的中间过程，以 role="step" 的 JSON 行存进历史，
 * 只作 UI 展示（可折叠卡片），loadHistory 会跳过，不喂给 LLM。
 */
export type ThinkStep = { type: "think"; content: string };
export type ToolStep = { type: "tool"; name: string; args: string; result?: string };
export type HistoryStep = ThinkStep | ToolStep;
...
export interface FlowContext {
  coreId: string;
  /** 本 core 是否为模板（目录位于项目 cores/ 搜索路径下）：模板可运行、可被 fork，但不可 commit */
  template: boolean;
  sessionId: string;
  userText: string;
