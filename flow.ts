...
export async function agentLoop(
  ...
  // 本轮 LLM 响应的过程日志（llm 统计 / 截断诊断）：只累积，最后由 putAssistant 并进 assistant 的
  // meta.steps 一起落库——不再有独立 step 行，渲染不再依赖 DB 顺序约定（2026-08-16 steps 归位）。
  let roundSteps: HistoryStep[] = [];
  const pushLog = (log: HistoryStep) => {
    logs.push(log);
    roundSteps.push(log);
  };
  /** 落一条 assistant：内存/LLM 侧存干净标准消息（meta 只存不喂）；DB 侧（live）把本轮过程日志并进
   *  meta.steps 一起写——日志随卡片走：渲染、删除、截断都跟着 assistant，孤儿不可能。 */
  const putAssistant = (assistant: ChatMessage) => {
    const steps = roundSteps;
    roundSteps = [];
    messages.push(assistant);
    if (live) ctx.memory.insert(steps.length ? { ...assistant, meta: { steps } } : assistant);
  };
  /** 无 assistant 可挂的日志（失败/空回复/终止）：live 时作为独立 role="step" 行落库（UI 独立展示）。 */
  const flushLogs = () => {
    if (live && roundSteps.length) {
      for (const log of roundSteps) ctx.memory.insert({ role: "step", content: JSON.stringify(log) });
    }
    roundSteps = [];
  };
  for (let step = 1; ; step++) {
    ...
    if (res.toolCalls.length === 0) {
      if (ctx.abortSignal.aborted) {
        ...
        // 只记 aborted 日志（并进 meta）。回复如实标"（已停止）"。
        if (res.reasoning) {
          if (res.content?.trim()) pushLog({ type: "cut", aborted: true });
          putAssistant({ role: "assistant", content: "", reasoning_content: res.reasoning });
        } else {
          flushLogs();
        }
        reply = "（已停止）";
      ...
      if (text) {
        // 最终回复：一条标准 assistant 消息（content + reasoning_content 一体，本轮日志并进 meta）落库
        putAssistant({ role: "assistant", content: res.content ?? "", ...(res.reasoning ? { reasoning_content: res.reasoning } : {}) });
        reply = text;
      } else {
        ...
        if (abnormal && res.reasoning) {
          ...
          ctx.emit(cut);
          resumeCount++;
          // 续跑：半截思考以 assistant 落库（cut 日志并进它的 meta——重试轮的内容）
          putAssistant({ role: "assistant", content: "", reasoning_content: res.reasoning });
          continue;
        }
        pushLog(cut);
    ...
    // 工具轮：一条标准 assistant 消息（content + reasoning_content + tool_calls 一体，本轮日志并进 meta，
    // 原样回传——DeepSeek 要求带 tools 的请求必须完整回传 reasoning_content，这是模型"继续之前的思考"的官方机制）
    putAssistant({
      role: "assistant",
      content: res.content ?? "",
    ...
    try {
      ...
      reply = `工具执行失败: ${msg}`;
      flushLogs(); // 该轮 assistant（含 llm 日志 meta）已落库；失败日志无主，独立成行
      break;
    }
    ...
    if (doneMsg) {
      ...
      putAssistant({ role: "assistant", content: doneMsg });
      reply = doneMsg;
      break;
    }
