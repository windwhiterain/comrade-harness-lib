...
const forkTool = async (args: { source: string; name?: string }, signal?: AbortSignal) => {
  // fork 不改源：允许 fork 任意 core 作来源。fork 直接基于源的最新提交——
...
const TOOLS: ToolDef[] = [
  ...
  },
  {
    ...
    function: {
      name: "fork_core",
      description:
        "从已有 core 复制出一个新 core（git clone：新 core 拥有源的全部 git 历史，之后独立演化）。源 core 不受影响。可以 fork 任意 core。fork 直接基于源的最新提交——源有未提交修改时不会进新 core（daemon 会带 warning），必要时先 snapshot。name 省略时默认 <source>-fork",
      parameters: {
        type: "object",
        properties: {
